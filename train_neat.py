...
# Complementary incentive: when the block directly ahead is TWO tiles tall
# (no room to jump over it — jumping structurally cannot clear it), holding
# jump is never going to help; only walking (to find a lower gap, a step
# down, or a way around) can. Observed: the network holds jump anyway for
# hundreds of frames in exactly this situation, apparently generalizing the
# "jump when blocked" habit somewhere it doesn't apply. This rewards NOT
# jumping while still trying to move, specifically when jumping provably
# can't help — it never fires in the (more common) case where jumping over
# is genuinely the right move, so it can't undermine that incentive.
NARROW_GAP_BONUS_PER_FRAME = 0.3
MAX_NARROW_GAP_BONUS_PER_EPISODE = 15

# After clearing an obstacle into open space, observed: the network keeps
# hopping in place (x_speed climbs a little each jump, then resets to 0 at
# the apex, over and over) instead of committing to a sustained run, even
# with a wide clear corridor ahead and nothing blocking it. None of the
# incentives above cover this — they all target *reacting to* an obstacle;
# none reward simply *maintaining* forward momentum once nothing is in the
# way. Rewarding raw speed every frame would trivially pay out for normal
...
def eval_genomes(genomes, config, env, render=False, initial_level_offset=0):
    for genome_id, genome in genomes:
        ...
        for step in range(MAX_STEPS_PER_EPISODE):
            ...
            # two-tile wall) — jumping structurally can't clear this, only
            # walking (to find a lower gap or a way around) can. Reward
            ...
            # for a while — pays nothing for the first few frames (so plain
            # walking or a single jump's brief speed isn't rewarded), only for
            # genuinely unbroken momentum, which the observed "hop, reset to 0,
            # hop again" tic never reaches.
            if x_speed_now > 0:
                running_streak += 1
            else:
                running_streak = 0
            if running_streak >= RUNNING_STREAK_THRESHOLD:
                running_bonus += RUNNING_BONUS_PER_FRAME
